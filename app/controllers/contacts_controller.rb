class ContactsController < ApplicationController
  ...
  def create
    ...
    if @contact.save
        ...
        #this is hash syntax. attempt to submit the form without filling in the email then check the logs in the running rails server tab in IDE. in essence, contact is a hash with name, email, and comments properties. below, we're defining three instance variables to the different aray properties within the contact hash. So, params[:contact][:name] would reference the name property within the contact hash.
      ...
      body = params[:contact][:comments]
        ...
        #calling the ContactMailer class that was defined in the contact_mailer.rb file, then calling thre arguments that are defined above (using hash notation)
